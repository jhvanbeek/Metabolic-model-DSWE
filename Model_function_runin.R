...
#concentration derivatives
  ...
  # the store_glc, stored glucose-derived metabolites, is calculated as moles per liter cytosolic water
  ...
  ADP_per_ml <- as.numeric( (p2ck["Vcyt"] * y["ADP"]) / 1000 * (1.-p2ck["DW_ratio"]) * p2ck["dens"])
    ...
    functionValues <- c(
                        NADH = y_calc_NADH,
                        AdN = as.numeric(y["ATP"]+y["ADP"]), # concentration adenine nucleotides minus AMP
                        Vhyd = Vhyd,
                        Vmit = Vmit,
                        Vhead = Vhead,
                        Vtail = Vtail,
